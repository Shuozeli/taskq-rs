...
//!
//! As of `pure-grpc-rs` commit `58c35d9` ("generate FlatBuffers gRPC stubs
//! end-to-end"), `compile_fbs` emits a sibling `<stem>_grpc.rs` containing:
//!
//! - top-level `pub use generated::<ns>::<Name>T as <Name>;` aliases for every
//!   reachable message and non-union enum, exposing the FlatBuffers Object API
//!   types (`Default`, `pack`, `unpack`) as the owned wrappers gRPC needs;
//! - `impl FlatBufferGrpcMessage` for every type used as an RPC input or
//!   output, bridging the codec to the Object API;
//! - per-service `<service>_server` modules with the trait + `tower::Service`
//!   adapter, named per `taskq.v1.<Service>`;
//! - per-service `<service>_client` modules with the typed client struct.
//!
//! The owned wrappers are accessible at the crate root (`taskq_proto::SubmitTaskRequest`)
//! while the zero-copy reader types remain at [`v1`] (`taskq_proto::v1::SubmitTaskRequest`).
//! Service traits live at `taskq_proto::task_queue_server::TaskQueue` etc.
//!
//! ## Schema discipline
//!
//! ## Schema discipline
...
// Owned wrappers + FlatBufferGrpcMessage impls + service traits + clients.
// Generated by pure-grpc-rs's grpc-build (see the module-level docstring above
// for the structure of this file).
include!(concat!(env!("OUT_DIR"), "/common_grpc.rs"));
